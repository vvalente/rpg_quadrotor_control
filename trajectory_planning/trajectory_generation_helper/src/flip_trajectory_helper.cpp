...
quadrotor_common::Trajectory computeFlipTrajectory(const quadrotor_common::TrajectoryPoint start_point,
  ...
  Eigen::Vector3d euler_angles_start = quadrotor_common::quaternionToEulerAnglesZYX(start_point.orientation);
  ...
  for (double dt_flip = 0.0; dt_flip < accel_time; dt_flip += dt) {
    ...
    curr_acceleration = Eigen::Vector3d(0.0, 0.0, mass_normalized_thrust - 9.81);
    ...
    point.orientation = quadrotor_common::eulerAnglesZYXToQuaternion(euler_angles);
  ...
  /**************************************
   * compute angular acceleration needed to perform the desired spin angle in the desired time
     we split the maneuver in two parts, angular acceleration phase and a symmetric deceleration phase
     use constant acceleration model -> rot_angle/2 = 1/2 a * (t/2)^2,
     where 'a' is the angular acceleration and 't' is the desired total spin time
   **************************************/
  ...
  for (double dt_flip = 0.0; dt_flip < flip_time; dt_flip += dt) {
    ...
    curr_acceleration = q_orientation * Eigen::Vector3d(0.0, 0.0, mass_normalized_thrust)
    ...
    point.angular_acceleration = Eigen::Vector3d(angular_acceleration, 0.0, 0.0);
    point.angular_jerk = Eigen::Vector3d(0.0, 0.0, 0.0);
    point.angular_snap = Eigen::Vector3d(0.0, 0.0, 0.0);

    Eigen::Vector3d thrust = curr_acceleration + Eigen::Vector3d(0.0, 0.0, 9.81);
    double inv_thrust = 1.0 / thrust.norm();
    point.jerk = (point.orientation.inverse() * (inv_thrust * Eigen::Matrix<double, 3, 3>::Identity()
        - thrust * thrust.transpose() / (inv_thrust * inv_thrust * inv_thrust))).inverse()
