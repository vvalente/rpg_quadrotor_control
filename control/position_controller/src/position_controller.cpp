...
quadrotor_common::ControlCommand PositionController::run(
  ...
  if (config.perform_aerodynamics_compensation) {
    ...
    command.collective_thrust -= config.k_thrust_horz
        * (pow(state_estimate.velocity.x(), 2.0)
            + pow(state_estimate.velocity.y(), 2.0));
  }

  const Eigen::Quaterniond desired_attitude = computeDesiredAttitude(
      desired_acceleration, reference_state.heading,
...
PositionController::computeNominalReferenceInputs(
  ...
  // C-frame: world frame rotated by desired heading
  ...
  if (almostZeroThrust(reference_command.collective_thrust)) {
    ...
    // TODO: fix, reference: https://www.flyingmachinearena.ethz.ch/wp-content/uploads/2012/fma-publications/hehn_dandrea_trajectory_generation_control.pdf
    // New version:
    Eigen::Matrix<double, 3, 3> outer_product = des_acc * des_acc.transpose();
    Eigen::Vector3d second_term = (-1.0 / pow(des_acc.norm(), 3)) * outer_product * reference_state.jerk;
//    reference_command.bodyrates.x() = -1.0 / reference_command.collective_thrust * y_B.dot(reference_state.jerk + second_term);
//    reference_command.bodyrates.y() = 1.0 / reference_command.collective_thrust * x_B.dot(reference_state.jerk + second_term);

    // Original version:
    reference_command.bodyrates.x() = -1.0 / reference_command.collective_thrust * y_B.dot(reference_state.jerk);
    reference_command.bodyrates.y() = 1.0 / reference_command.collective_thrust * x_B.dot(reference_state.jerk);
  }

  if (almostZero((y_C.cross(z_B)).norm())) {
    reference_command.bodyrates.z() = 0.0;
  } else {
    reference_command.bodyrates.z() = 1.0 / (y_C.cross(z_B)).norm()
        * (reference_state.heading_rate * x_C.dot(x_B) + reference_command.bodyrates.y() * y_C.dot(z_B));
  }
  ...
  if (almostZeroThrust(reference_command.collective_thrust)) {
    ...
    reference_command.angular_accelerations.x() = -1.0 / reference_command.collective_thrust
        * (y_B.dot(reference_state.snap) + 2.0 * thrust_dot * reference_command.bodyrates.x()
            - reference_command.collective_thrust * reference_command.bodyrates.y() * reference_command.bodyrates.z());
    reference_command.angular_accelerations.y() = 1.0 / reference_command.collective_thrust
        * (x_B.dot(reference_state.snap) - 2.0 * thrust_dot * reference_command.bodyrates.y()
  ...
  if (almostZero((y_C.cross(z_B)).norm())) {
    ...
    reference_command.angular_accelerations.z() = 1.0 / (y_C.cross(z_B)).norm()
        * (reference_state.heading_acceleration * x_C.dot(x_B)
            + 2.0 * reference_state.heading_rate * reference_command.bodyrates.z() * x_C.dot(y_B)
            - 2.0 * reference_state.heading_rate * reference_command.bodyrates.y() * x_C.dot(z_B)
...
void PositionController::computeAeroCompensatedReferenceInputs(
  ...
  const Eigen::Vector3d velocity_in_body = state_estimate.orientation.inverse() * reference_state.velocity;
  ...
  const Eigen::Vector3d alpha = reference_state.acceleration - kGravity_ + dx * reference_state.velocity;
  const Eigen::Vector3d beta = reference_state.acceleration - kGravity_ + dy * reference_state.velocity;
  const Eigen::Vector3d gamma = reference_state.acceleration - kGravity_ + dz * reference_state.velocity;
  ...
  if (almostZero(y_B.norm())) {
    const Eigen::Vector3d z_B_estimated = state_estimate.orientation * Eigen::Vector3d::UnitZ();
  ...
  const double B1 = reference_command.collective_thrust - (dz - dx) * z_B.dot(reference_state.velocity);
  const double C1 = -(dx - dy) * y_B.dot(reference_state.velocity);
  const double D1 = x_B.dot(reference_state.jerk) + dx * x_B.dot(reference_state.acceleration);
  const double A2 = reference_command.collective_thrust + (dy - dz) * z_B.dot(reference_state.velocity);
  const double C2 = (dx - dy) * x_B.dot(reference_state.velocity);
  const double D2 = -y_B.dot(reference_state.jerk) - dy * y_B.dot(reference_state.acceleration);
  ...
  if (almostZero(denominator)) {
    ...
    if (almostZero(A2)) {
      ...
      reference_command.bodyrates.x() =
          (-B1 * C2 * D3 + B1 * C3 * D2 - B3 * C1 * D2 + B3 * C2 * D1) / (A2 * denominator);
    }
    ...
    const double thrust_dot =
        z_B.dot(reference_state.jerk) + reference_command.bodyrates.x() * (dy - dz) * y_B.dot(reference_state.velocity)
            + reference_command.bodyrates.y() * (dz - dx) * x_B.dot(reference_state.velocity)
            + dz * z_B.dot(reference_state.acceleration);

    const Eigen::Matrix3d omega_hat = quadrotor_common::skew(
        reference_command.bodyrates);
    const Eigen::Vector3d xi = R_W_B_ref
        * (omega_hat * omega_hat * D + D * omega_hat * omega_hat + 2.0 * omega_hat * D * omega_hat.transpose())
        * R_W_B_ref.transpose() * reference_state.velocity
        + 2.0 * R_W_B_ref * (omega_hat * D + D * omega_hat.transpose()) * R_W_B_ref.transpose()
            * reference_state.acceleration + R_W_B_ref * D * R_W_B_ref.transpose() * reference_state.jerk;

    const double E1 = x_B.dot(reference_state.snap) - 2.0 * thrust_dot * reference_command.bodyrates.y()
        - reference_command.collective_thrust * reference_command.bodyrates.x() * reference_command.bodyrates.z()
        + x_B.dot(xi);
    const double E2 = -y_B.dot(reference_state.snap) - 2.0 * thrust_dot * reference_command.bodyrates.x()
        + reference_command.collective_thrust * reference_command.bodyrates.y() * reference_command.bodyrates.z()
        - y_B.dot(xi);
    const double E3 = reference_state.heading_acceleration * x_C.dot(x_B) + 2.0 * reference_state.heading_rate
        * (reference_command.bodyrates.z() * x_C.dot(y_B) - reference_command.bodyrates.y() * x_C.dot(z_B))
        - reference_command.bodyrates.x()
            * (reference_command.bodyrates.y() * y_C.dot(y_B) + reference_command.bodyrates.z() * y_C.dot(z_B));

    if (almostZero(A2)) {
      reference_command.angular_accelerations.x() = 0.0;
    } else {
      reference_command.angular_accelerations.x() =
          (-B1 * C2 * E3 + B1 * C3 * E2 - B3 * C1 * E2 + B3 * C2 * E1) / (A2 * denominator);
    }
    reference_command.angular_accelerations.y() = (-C1 * E3 + C3 * E1) / denominator;
    reference_command.angular_accelerations.z() = (B1 * E3 - B3 * E1) / denominator;
  }
  ...
  const Eigen::Matrix3d bodyrates_est_hat = quadrotor_common::skew(
      state_estimate.bodyrates);

  reference_command.bodyrates = R_trans * bodyrates_ref;
  reference_command.angular_accelerations =
      R_trans * angular_accelerations_ref - bodyrates_est_hat * R_trans * bodyrates_ref;

  *reference_inputs = reference_command;

  // Drag accelerations
  *drag_accelerations = -1.0 * (R_W_B_ref * (D * (R_W_B_ref.transpose() * reference_state.velocity)));
}
...
Eigen::Vector3d PositionController::computePIDErrorAcc(
  ...
  // x acceleration
  double x_pos_error = reference_state.position.x() - state_estimate.position.x();
  quadrotor_common::limit(&x_pos_error, -config.pxy_error_max,
                          config.pxy_error_max);

  double x_vel_error = reference_state.velocity.x() - state_estimate.velocity.x();
  ...
  // y acceleration
  double y_pos_error = reference_state.position.y() - state_estimate.position.y();
  quadrotor_common::limit(&y_pos_error, -config.pxy_error_max,
                          config.pxy_error_max);

  double y_vel_error = reference_state.velocity.y() - state_estimate.velocity.y();
  ...
  // z acceleration
  double z_pos_error = reference_state.position.z() - state_estimate.position.z();
  quadrotor_common::limit(&z_pos_error, -config.pz_error_max,
                          config.pz_error_max);

  double z_vel_error = reference_state.velocity.z() - state_estimate.velocity.z();
...
double PositionController::computeDesiredCollectiveMassNormalizedThrust(
    ...
    const PositionControllerParams &config) const {
  const Eigen::Vector3d body_z_axis = attitude_estimate * Eigen::Vector3d::UnitZ();
...
Eigen::Quaterniond PositionController::computeDesiredAttitude(
  ...
  const Eigen::Vector3d x_B = computeRobustBodyXAxis(x_B_prototype, x_C, y_C,
                                                     attitude_estimate);
...
Eigen::Vector3d PositionController::computeRobustBodyXAxis(
  ...
  if (almostZero(x_B.norm())) {
    ...
    const Eigen::Vector3d x_B_estimated = attitude_estimate * Eigen::Vector3d::UnitX();
