    ...
    const Eigen::Quaterniond& attitude_estimate) const
{
  ...
  {
    reference_command.bodyrates.x() = -1.0 / reference_command.collective_thrust
        * y_B.dot(reference_state.jerk);
    reference_command.bodyrates.y() = 1.0 / reference_command.collective_thrust
  ...
  else
  {
    ...
    reference_command.angular_accelerations.x() = -1.0
    ...
    reference_command.angular_accelerations.y() = 1.0
        / reference_command.collective_thrust
    ...
    Eigen::Vector3d* drag_accelerations) const
{
  ...
  const Eigen::Vector3d velocity_in_body = state_estimate.orientation.inverse()
  ...
  {
    const Eigen::Vector3d z_B_estimated = state_estimate.orientation
  ...
  else
  {
    ...
    {
      reference_command.bodyrates.x() = (-B1 * C2 * D3 + B1 * C3 * D2
          - B3 * C1 * D2 + B3 * C2 * D1) / (A2 * denominator);
    }
    ...
    {
      reference_command.angular_accelerations.x() = (-B1 * C2 * E3
          + B1 * C3 * E2 - B3 * C1 * E2 + B3 * C2 * E1) / (A2 * denominator);
    }
    reference_command.angular_accelerations.y() = (-C1 * E3 + C3 * E1)
        / denominator;
    reference_command.angular_accelerations.z() = (B1 * E3 - B3 * E1)
        / denominator;
  }
  ...
  reference_command.angular_accelerations = R_trans * angular_accelerations_ref
      - bodyrates_est_hat * R_trans * bodyrates_ref;

  *reference_inputs = reference_command;

  // Drag accelerations
  *drag_accelerations = -1.0
    ...
    const PositionControllerParams& config) const
{
  ...
  // y acceleration
  double y_pos_error = reference_state.position.y()
      - state_estimate.position.y();
  quadrotor_common::limit(&y_pos_error, -config.pxy_error_max,
                          config.pxy_error_max);

  double y_vel_error = reference_state.velocity.y()
  ...
  // z acceleration
  double z_pos_error = reference_state.position.z()
      - state_estimate.position.z();
  quadrotor_common::limit(&z_pos_error, -config.pz_error_max,
                          config.pz_error_max);

  double z_vel_error = reference_state.velocity.z()
...
double PositionController::computeDesiredCollectiveMassNormalizedThrust(
    ...
    const PositionControllerParams& config) const
{
  const Eigen::Vector3d body_z_axis = attitude_estimate
      * Eigen::Vector3d::UnitZ();
    ...
    const Eigen::Quaterniond& attitude_estimate) const
{
  ...
  if (almostZero(x_B.norm()))
  {
    ...
    const Eigen::Vector3d x_B_estimated = attitude_estimate
        * Eigen::Vector3d::UnitX();
    const Eigen::Vector3d x_B_projected = x_B_estimated
        - (x_B_estimated.dot(y_C)) * y_C;
